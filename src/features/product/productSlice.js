...
export const productsSlice = createSlice({
  ...
  initialState: {
    products: [],
    selectedProduct: null, // for single product
    cartItems: [],
    wishlistItems: [],
    status: {
      fetchStatus: "idle",
    ...
    filters: {
      priceRange: { min: 0, max: 8000 }, // Based on your product prices
      selectedCategories: [], // For checkbox selections
      selectedRating: 0, // 0 means no filter
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(updateWishlistAsync.fulfilled, (state, action) => {
        ...
        // Update both products and wishlistItems arrays
      ...
      .addCase(updateCartAsync.fulfilled, (state, action) => {
        ...
        // Update both products and cartItems arrays
        const index = state.products.findIndex(
          (p) => p._id === action.payload._id
        );
        if (index !== -1) {
