...
const Navbar = () => {
 ...
 const renderMobileDrawer = () => (
   <Drawer anchor="right" open={isDrawerOpen} onClose={toggleDrawer}>
     <Box sx={{ width: 250, bgcolor: "#1a1a1a", height: "100%" }} role="presentation">
       <List>
         {["Profile", "Favorites", "Cart"].map((text, index) => (
           <ListItem
             button
             component={Link}
             to={text === "Profile" ? "/profile" : text === "Favorites" ? "/wishlist" : "/cart"}
             key={text}
             sx={{ color: "white" }}
             onClick={toggleDrawer}
           >
             <IconButton sx={{ color: "white" }}>
               {index === 0 ? <ProfileIcon /> : index === 1 ? <FavoriteIcon /> : <CartIcon />}
             </IconButton>
             {text}
           </ListItem>
         ))}
